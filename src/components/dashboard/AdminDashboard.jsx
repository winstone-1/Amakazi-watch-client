    ...
    <div className="space-y-6 transition-colors duration-300">
          ...
          <div className="flex items-center gap-4">
            <div className="h-16 w-16 rounded-full bg-primary/20 flex items-center justify-center text-primary border-2 border-primary/30 overflow-hidden">
              {user?.avatar ? (
                ...
                <span className="text-2xl font-black">{user?.username?.[0]?.toUpperCase() || 'A'}</span>
              )}
            </div>
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl font-black text-secondary dark:text-white">Welcome, {user?.username || 'System Administrator'}.</h1>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <AnimatedCard className="p-5">
            ...
            <p className="mt-2 text-3xl font-black text-primary">{stats.total_users || 1205}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <AnimatedCard className="p-5">
            ...
            <p className="mt-2 text-3xl font-black text-accent">{stats.total_reports || 483}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <AnimatedCard className="p-5">
            ...
            <p className="mt-2 text-3xl font-black text-emerald-600 dark:text-emerald-400">{stats.total_orgs || 42}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <AnimatedCard className="p-5">
            ...
            <p className="mt-2 text-3xl font-black text-amber-600 dark:text-amber-400">{stats.pending_verifications || 5}</p>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <AnimatedCard className="p-6">
          ...
          <div className="space-y-3">
            {[
              { id: 1, username: 'jane_doe', email: 'jane@example.com', role: 'survivor' },
              { id: 2, username: 'mwangi_k', email: 'mk@example.com', role: 'org_staff' },
              { id: 3, username: 'sarah_w', email: 'sarah@example.com', role: 'counselor' },
            ].map((user) => (
              <div key={user.id} className="flex items-center gap-3 rounded-2xl border border-slate-200/70 bg-white/60 p-3 dark:border-white/10 dark:bg-slate-800/60">
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-secondary dark:text-white">{user.username}</p>
        ...
        <AnimatedCard className="p-6">
          ...
          <div className="space-y-3">
            {[
              { id: '1092', created_at: '2 hours ago' },
              { id: '1091', created_at: '5 hours ago' },
              { id: '1088', created_at: '1 day ago' },
            ].map((report) => (
              <div key={report.id} className="flex items-start gap-3 rounded-2xl border border-slate-200/70 bg-white/60 p-3 dark:border-white/10 dark:bg-slate-800/60">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">{report.created_at}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <AnimatedCard className="p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {[
            { id: 1, name: 'Hope Center Nairobi', county: 'Nairobi' },
            { id: 2, name: 'Safe Haven Mombasa', county: 'Mombasa' },
          ].map((org) => (
            <div key={org.id} className="flex items-center gap-3 rounded-2xl border border-slate-200/70 bg-white/60 p-3 dark:border-white/10 dark:bg-slate-800/60">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-secondary dark:text-white">{org.name}</p>
                <p className="text-sm text-slate-500 dark:text-slate-400">{org.county}</p>
              </div>
